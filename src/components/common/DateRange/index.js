  ...
  render () {
    ...
    return (
      <div className="daterange-custom" style={{ ...style }} key={this.nextResetKey}>
        <DatePicker getCalendarContainer={triggerNode => triggerNode.parentNode} disabled={disabledArr[0]} { ...obj} value={startValue}  format={format} placeholder={placeholder[0]} style={{ ...itemStyle }} disabledDate={(date) => {return  this.disabledDate('start', date)}} onChange={(date)=>{ this.onChange('start', date)}}/>
        <span className="split">-</span>
        <DatePicker className="end-time" getCalendarContainer={triggerNode => triggerNode.parentNode} disabled={disabledArr[1]} { ...obj} value={endValue} format={format} placeholder={placeholder[1]} style={{ ...itemStyle }} disabledDate={(date) => {return  this.disabledDate('end', date)}} onChange={(date)=>{ this.onChange('end', date)}}/>
      </div>
    )
  }
}
